...
This guide assumes the manual Docker Compose workflow where you maintain your own `docker-compose.yml`.

If you are using the repository `Makefile` workflow, keep in mind that `/opt/wg-easy/docker-compose.yml` is generated by `make config` and will be overwritten on the next render unless you also preserve your changes in the source deployment files.
